...
class DataUtils():
    ...
    def get_test_data(self, loc='data/minitest.csv'):
        file = open(loc)
        lines = file.read().splitlines()
        return_var = []
        for line in lines:
            line_split = line.split(';')
            return_var.append(list(map(str, line_split[:-1])) + [line_split[-1]])
        return return_var

    def get_train_data(self, loc='data/minitrain.csv'):
        file = open(loc)
        lines = file.read().splitlines()
        return_var = []
        for line in lines:
            ...
            def natural_type(x):
                try:
                    ...
                    ret = x
            return_var.append(list(map(natural_type, line_split[:-1])) + [line_split[-1]])
        return return_var
...
class DecisionTree():
    ...
    def print_tree(self, tree, indent='', hm=None):
        string = ''
        ...
        if tree.results != None:
            ...
            #print(tree.col)
            #print(tree.nos[1] / float(sum(tree.nos)))
            #print(tree.nos[0] / float(sum(tree.nos)))

            if tree.col not in hm:
                hm[tree.col] = 'Place_holder'+str(tree.col)
            ...
            # string += the branches
            string += (indent + 'True :->')
            string += self.print_tree(tree.left, indent + '|  ', hm=hm)
            string += (indent + 'False :->')
            string += self.print_tree(tree.right, indent + '|  ', hm=hm)
        return string
    ...
    def confusion_matrix(self, predicted_label, true_label):
        pred_list = predicted_label#[i[0] for i in map(list, predicted_label)]
        true_list = true_label#[single for single in true_label]
