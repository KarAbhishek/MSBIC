...
def gibbs_motif_search(dna, k, t, N):
    ...
    for w in range(N):
        ...
        pro_file = create_profile(motif[:y]+motif[y+1:])
        motif = motif[:y]+[most_probable_kmer_given_profile(dna[y], k, pro_file)]+motif[y+1:]
        scor = score(motif)
        if scor < best_score:
            best_motif = motif[:]
            best_score = scor
...
the_one = []
for i in range(20):
    b1, b2 = gibbs_motif_search(DNA, K, T, N)
    if b1<min_b1:
        min_b1 = b1
        the_one = b2
