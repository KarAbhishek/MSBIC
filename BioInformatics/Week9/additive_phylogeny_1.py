...
def recurse(nn, stable_n, dist_mat):
    ...
    limb_l = limb_len(nn - 1, dist_mat)
    dist_mat_bald = create_bald(dist_mat, limb_l, nn-1)
    (i, k) = perfect_leaves(dist_mat_bald, nn - 1)
    x = dist_mat_bald[i][nn - 1]
    dist_mat_trim = create_trim(dist_mat, nn-1)
    T = recurse(nn - 1, stable_n, dist_mat_trim)
...
if __name__ == '__main__':
    file_ob = open('additive_data.txt')
